(() => {
  ...
  const RANDOM_KEY = "wordly_random_v1";
  const LAST_MODE_KEY = "wordly_last_mode_v1";
  const DAILY_PREFIX = "wordly_daily_v1_";
  ...
  function sanitizeLoadedState(raw, mode, solution, stamp, randomId) {
    ...
    return {
      ...
      currentGuess: typeof raw.currentGuess === "string" ? raw.currentGuess.toUpperCase().replace(/[^A-Z]/g, "").slice(0, WORD_LENGTH) : "",
  ...
  function loadRandomState(forceNew) {
    if (!forceNew) {
      ...
      if (raw && typeof raw.solution === "string" && /^[A-Z]{5}$/.test(raw.solution)) {
        return sanitizeLoadedState(raw, "random", raw.solution, null, raw.randomId || null);
      }
    }
  ...
  function renderMeta() {
    ...
    const puzzleLabel = state.mode === "daily" ? `#${state.stamp}` : `${state.randomId?.slice(-6) || "local"}`;
  ...
  function shareResult() {
    ...
    const puzzle = state.mode === "daily" ? `Daily ${state.stamp}` : `Random ${state.randomId?.slice(-6) || "local"}`;
    ...
    const text = `WORDLY ${puzzle} ${score}\n${grid}\n${location.origin}${location.pathname}`;
